import { listCategories, type Category } from "@/lib/categories";

export const dynamic = "force-dynamic";

export default async function CategoriesPage() {
  let categories: Category[] = [];
  let error: string | null = null;
  try {
    categories = await listCategories();
  } catch (e) {
    error = e instanceof Error ? e.message : String(e);
  }

  return (
    <main>
      <div className="crumb">
        <a href="/">← Project Kenny</a> / categories
      </div>
      <div className="kicker">fastrak reference data</div>
      <h1>Categories</h1>

      {error ? (
        <div className="card" style={{ padding: "18px 20px" }}>
          <strong style={{ color: "#f0a3a3" }}>Database not reachable.</strong>
          <p className="muted" style={{ marginBottom: 6 }}>
            Start it and load the data with: <code>npm run db:setup</code>
          </p>
          <p className="muted" style={{ fontSize: 12, marginBottom: 0 }}>
            ({error})
          </p>
        </div>
      ) : (
        <>
          <div className="badge-row">
            <span className="count">{categories.length} category(ies)</span>
            <span className="tag">source: category.dbf</span>
            <a href="/categories/new" className="tag">
              + new category
            </a>
          </div>
          <div className="card">
            <table>
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Category</th>
                  <th>Remarks</th>
                  <th></th>
                </tr>
              </thead>
              <tbody>
                {categories.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="muted">
                      No categories yet — run <code>npm run db:import</code> or add one.
                    </td>
                  </tr>
                ) : (
                  categories.map((c) => (
                    <tr key={c.id}>
                      <td style={{ fontFamily: "var(--mono)", fontSize: 12 }}>{c.id}</td>
                      <td>{c.category ?? "—"}</td>
                      <td>{c.remarks ?? "—"}</td>
                      <td>
                        <a href={`/categories/${c.id}`}>edit</a>
                      </td>
                    </tr>
                  ))
                )}
              </tbody>
            </table>
          </div>
        </>
      )}
    </main>
  );
}
